...
enum class TamanioUnidad
{
    Pequeno,
    Mediano,
    Grande
};
enum class TipoDanoAereo
{
    Normal,
    Explosivo,
    Contusivo
};
enum class TipoDanoTerrestre
{
    Normal,
    Explosivo,
    Contusivo
...
int main()
{
    ...
    while (!HeoresSquad1.empty() && !HeoresSquad2.empty())
    {
        ...
        cout << "Los heroes de " << jugador1 << " son: " << endl;
        /* for(auto i: HeoresSquad1){
            cout<<"\t"<<i->getNombre()<<endl;
            archivo<<"\t"<<i->getNombre()<<endl;

        }
        cout << "Los heroes de " << jugador2 << " son: " << endl;
        for(auto i: HeoresSquad2){
            cout<<"\t"<<i->getNombre()<<endl;
            archivo<<"\t"<<i->getNombre()<<endl;
        } */ 
            ...
            if (enemigoMaxVida1 != HeoresSquad2.end() && (*enemigoMaxVida1)->getVida() > 0)
            {
                ...
                else if ((*enemigoMaxVida1)->getRaza() == Raza::Zerg)
                {
                    ...
                    if (enemigoZerg->getAutodestruccion() == true)
                    {
                        ...
                        cout << unidad->getNombre() << "  - Vida " << unidad->getVida() << endl;
                        ...
                        archivo << unidad->getNombre() << " - Vida " << unidad->getVida() << endl;
                    }
                    // primer ataque
